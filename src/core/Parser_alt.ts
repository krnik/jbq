import { SYM_SCHEMA_COLLECTION, SYM_SCHEMA_CONFIG, SYM_SCHEMA_PROPERTIES, TYPE, SYM_TYPE_NAME, SYM_TYPE_VALIDATE, SYM_TYPE_EXTERNAL, SYM_TYPE_FOR_LOOP } from '../constants';
import { IType, TypeWrapper } from '../types/Wrapper';
import { E, debug, is } from '../utils/index';
    ...
    parseSchema (types: TypeWrapper, schema: ISchema, conf: IConfig, names: INames, source: ISource) {
        ...
        if (schema.hasOwnProperty(SYM_SCHEMA_COLLECTION)) {
            ...
            if (!type[SYM_TYPE_FOR_LOOP]) {
                source.code += `
if (!(Symbol.iterator in ${names.var}))
