...
export class CodeGenerator {
     ...
     */
    public static renderCloseBlock (): string { return '}'; }
    ...
    public static renderIfStatement (
        ...
        if (conditions.length === 0)
            throw CodeGenerator.Error.emptyIfConditionArray();
        return `${Keyword.If} (${
            conditions
    ...
    public static renderForOfLoop (variableName: string, iterable: string, path: string): string {
        ...
        return `${CodeGenerator.renderIfStatement([ifCondition])}
                ${CodeGenerator.renderReturnJSONMessage(`Data requires to have ${Symbol.iterator.toString()} method implemented in order to use for..of loop`, path)}
     ...
     */
    public static renderForLoop (variableName: string, collection: string, accessor: string): string {
        return `
            ${CodeGenerator.renderVariableInitialization(`${collection}_len`, collection, '.length')}
            ${Keyword.For} (
                ${CodeGenerator.renderVariableInitialization(accessor, '0', '', Keyword.Let)}
                ${accessor} ${ComparisonOperator.LessThan} ${collection}_len;
                ${accessor}++
            ) {
                ${CodeGenerator.renderVariableInitialization(variableName, collection, `[${accessor}]`)}`;
    }
    ...
    public static renderFunctionCall (
        ...
        return `${Keyword.Const} ${resultVariableName} = ${fnParam}(${schemaValue}, ${TypeReflect.asString(schemaPath)}, ${variableName});
     ...
     */
    public static renderDataPathResolution (dataPath: string | string[], variableName: string): string {
        ...
        if (!paths.length)
            throw CodeGenerator.Error.invalidDataPath(dataPath);
    ...
    public static renderDataPath (dataPath: string | string[]): string {
        return `(${Array.isArray(dataPath)
            ? dataPath.join(SCHEMA_PATH_SEPARATOR)
            : dataPath})`;
    }
